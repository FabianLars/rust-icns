...
fn adjust_png_to_match_png_encoder_quirks(image: Image) -> Image {
    if image.pixel_format() != PixelFormat::PNG {
        return image;
    }
    // Force decoding from PNG.
    Image::decode_from_png(image.data()).unwrap()
}

fn encoder_test(png_name: &str, icon_type: IconType, icns_name: &str) {
    let image = load_png_file(png_name).unwrap();
    let image = adjust_png_to_match_png_encoder_quirks(image);
